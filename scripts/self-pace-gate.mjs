...
// The `at` branch additionally requires an argument shaped like real at(1) usage
// (a timespec word, a number, or an option) -- a bare `at\b` matched every `at`
// substring that landed at a fake segment start after an escaped-\| split (grep
// alternations like `at\|be`, regex tails like `at\b`), 3 false denies on
// 2026-07-13 (card 41b8741f). crontab/launchctl/systemd-run/batch keep the
// bare-word form: they have no such substring collisions in practice.
const SCHED_BOUNDARY = '[;&|(`]'
const SCHEDULER_RX = new RegExp(String.raw`(^|${SCHED_BOUNDARY}\s*)${SCHED_PREFIX}(?:(?:crontab|launchctl|systemd-run|batch)\b(?!-)(?!\s*=)|at\s+(?:-[A-Za-z]|\d|now\b|noon\b|midnight\b|teatime\b|next\b|today\b|tomorrow\b))`, 'i')
