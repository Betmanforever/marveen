...
**Verzió:** 1.13 (v1.0-1.7: 2026-07-30 14:45-16:40; v1.8: 17:00, horgony-szabály; v1.9: 2026-07-31, linguist-kapu; v1.10: 2026-08-01, preCheck 8.1 szakasz + PUT-mérés javítás + neo néma-Sonnet eset; v1.11: 2026-08-01 12:05, startup-modell-kapu fix + token-rotáció; v1.12: 2026-08-01 21:55, WSL2 memória/swap plafon + a boot-fire napindító eset; v1.13: 2026-08-02 06:40, napindító hot-memória szekció + hot tier takarítás)
...
|---|---|---|---|---|
| 2026-08-02 06:40 | `reggeli-napindito` SKILL.md: új, kötelező ELSŐ szekció, amely a `hot` memóriából kiolvassa a legfrissebb konszolidált reggeli összefoglalót és `📌 DÖNTÉSRE VÁR` címmel az üzenet élére teszi. Fail-safe: hiányzó vagy 24 óránál régebbi bejegyzésnél csendben kimarad. Mellette a `hot` tier takarítva: 9 felülírt/idejétmúlt bejegyzés törölve, a lekérdezés 10 találatról 1-re | `[MÉRT]` A napindító prompt-fájlja az emailt, naptárt, AI híreket, a DREAM.md-t és a hétfői riportot ismerte, a `hot` memóriát **nem**. Az éjszakai flotta-munka eredménye viszont oda gyűlik, tehát a task hibátlanul lefutott volna egy jól formázott, de a lényeget nem tartalmazó üzenettel. Nincs hozzá hibajel: a néma no-op feltűnő, ez nem az. A takarítás azért része a fixnek, mert a beírt lekérdezés futtatva 10 rendezetlen, részben egymásnak ellentmondó találatot adott | mr-wolfe | `SKILL.md.bak-20260802` visszamásolása a task könyvtárában; a törölt memóriák tartalma a napi naplóban és a `1102`-es bejegyzésben megmaradt |
| 2026-08-01 21:35 | WSL2 erőforrás-plafon a `.wslconfig`-ban: explicit `memory=8GB`, swap 2 GB-ról 8 GB-ra. `[MÉRT 21:49]` swap 8.0Gi él, a VM 8 perce fut, a flotta visszaállt | 21:08 és 21:29 között a VM memória-kimerülés miatt teljesen befagyott (hat Claude session + Ollama + dashboard + MCP-k egy 7,7 GB-os VM-en, kernel page-fault viharban dobott log-sorokkal). Swap nélkül az üresjárati ágens-memória nem tud kilapozódni, ezért a kernel megállt, nem lassult | Szabó Gábor (host oldal, Windows), mérés és beemelés mr-wolfe | `.wslconfig` visszaállítása + `wsl --shutdown`; a swap-emelést nem javasolt visszavenni |
| 2026-08-01 21:30 | `[MÉRT]` A host reboot elindította a `mr-wolfe-morning.service`-t 21:30:31-kor; a napindító csak azért nem ment ki este fél tízkor, mert az OOM megölte a processzt 21:31:05-kor (978M peak). Fix folyamatban (`40cadf14`, neo): a `mr-wolfe-morning.timer` `[Unit]` szekciójából a felesleges `Requires=` sor törlése, plusz ugyanez a minta végignézve a többi timeren | A timer `[Unit]` `Requires=` dependenciája **boot**kor azonnal elindítja a service-t, az `OnCalendar`-tól függetlenül; a timer→service kötést a `[Timer]` `Unit=` adja, tehát a sor felesleges és káros. Következmény: bármely éjszakai reboot kiküld egy napindítót, akár hajnali 3-kor, ami a 22:00-06:00 csendes sáv közvetlen sérülése. Ma csak az OOM fedte el, ez szerencse volt és nem kontroll | észlelés mr-wolfe, fix neo | a `Requires=` sor visszaírása + `daemon-reload` |
