...
export function initDatabase(dbPathOverride?: string): void {
  ...
  // The recreated table carries the estimate-tracking columns added just above
  // (estimated_hours, estimated_by, work_started_at, work_completed_at) in both
  // its schema and the INSERT..SELECT, so an install that already has estimate
  // data does NOT lose it when the testing-status recreate fires.
  try {
      ...
      db.exec(`
        ...
        INSERT INTO kanban_cards_new
          SELECT id, title, description, status, assignee, priority, project, due_date,
                 ...
                 estimated_hours, estimated_by, work_started_at, work_completed_at
          FROM kanban_cards;
...
export function moveKanbanCard(id: string, status: KanbanCard['status'], sortOrder: number, actor?: string): boolean {
  ...
  // Work-time tracking (ours): stamp work_started_at / work_completed_at on the
  // relevant transition; the fetched card already gives us the previous status.
  ...
  // Audit trail (theirs): record an event only on a real status transition,
  // not a pure sort_order reorder within the same column.
  if (changed && prev !== status) {
    db.prepare(
